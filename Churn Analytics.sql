LOAD DATA INFILE 'C:\\ProgramData\\MySQL\\MySQL Server 8.0\\Uploads\\mta-customer-feedback-data.csv'
INTO TABLE data
...
DROP TABLE IF EXISTS copy_of_data;
CREATE TABLE copy_of_data SELECT DISTINCT * FROM data;
DROP TABLE data;
ALTER TABLE copy_of_data RENAME TO data;
...
SELECT COUNT(Feedback) AS Retained_Customers_Count
FROM data 
WHERE Feedback = "Commendation" ;
...
WITH Complaints_CTE AS (SELECT COUNT(Feedback) AS Number_of_Complaints FROM data WHERE Feedback = 'Complaint')
SELECT CONCAT(ROUND(((Number_of_Complaints)*100/(SELECT COUNT(Feedback) FROM data)), 2), "%") AS Churning_Rate_of_Customers
FROM Complaints_CTE;
...
SELECT COUNT(Feedback) AS Churned_Customers_Count
FROM data 
